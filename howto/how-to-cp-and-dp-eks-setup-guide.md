...
> - Gateway API controller support for Control Plane and Control Tower data planes; EKS users can evaluate NGINX Gateway Fabric, Traefik, Istio, NetScaler, and other supported Gateway API controllers where capabilities support them
...
| `TP_GATEWAY_API_ENABLED` | `false` | Optional evaluation flag for Gateway API examples |
| `TP_GATEWAY_NAMESPACE` | `nginx-gateway` | Namespace for the Gateway API controller and Gateway resource |
| `TP_GATEWAY_NAME` | `nginx-gateway` | Gateway resource name referenced by TIBCO `HTTPRoute` objects |
| `TP_GATEWAY_CLASS` | `nginx` | GatewayClass name; NGINX Gateway Fabric uses `nginx` |
...
#### Why `dnsTunnelDomain` Still Exists in Simplified DNS

TIBCO Platform uses two routing roles even when they share one DNS base domain:

- `global.external.dnsDomain` is the public domain for normal Control Plane router traffic, such as the Platform Console, subscription portal, and platform APIs.
- `global.external.dnsTunnelDomain` is the public domain that represents hybrid connectivity tunnel traffic handled by `hybrid-proxy`.

In the simplified DNS model, both values intentionally point to `${TP_BASE_DNS_DOMAIN}`. The distinction is still important because the Control Plane and Data Plane registration flows need to know which public DNS domain should be advertised for tunnel connectivity. With the baseline Ingress model, the workshop routes tunnel traffic on the subscription host under `/infra/tunnel`, so no separate tunnel DNS zone or certificate is needed:

```text
https://${CP_SUBSCRIPTION}.${TP_BASE_DNS_DOMAIN}/infra/tunnel
```

In the legacy model, `dnsTunnelDomain` points to a separate tunnel domain such as `${CP_INSTANCE_ID}-tunnel.${TP_HOSTED_ZONE_DOMAIN}`. In the optional Gateway API example later in this guide, a separate tunnel hostname is also used because the current `tibco-cp-base` Gateway API `HTTPRoute` values create root-path routes rather than the simplified `/infra/tunnel` path split.
...
Gateway API is the Kubernetes successor to the older `Ingress` API. Instead of a single `Ingress` object, it separates responsibilities into `GatewayClass`, `Gateway`, and route resources such as `HTTPRoute`:

- `GatewayClass` identifies the controller implementation, such as NGINX Gateway Fabric.
- `Gateway` represents the listener and load-balancer entry point.
- `HTTPRoute` attaches application routes to a Gateway and points traffic to Kubernetes services.

The official TIBCO Platform requirements list Gateway API controller support for both Control Plane and data plane usage. For TIBCO Platform 1.18.0, supported Gateway API controllers include NGINX Gateway Fabric 2.3.0, Istio 1.28.2, Traefik `traefik-39.0.7`, NetScaler `netscaler-cpx-with-gateway-controller-2.0.0`, and the `Other Gateway API Controller` option for data plane controllers that are not listed explicitly.

For this EKS workshop, keep the default ALB plus Nginx or Traefik ingress path unless you are intentionally validating Gateway API. Gateway API changes the Kubernetes resources created for endpoint exposure, but it does not remove the need for DNS, certificates, network policies, storage, PostgreSQL, or the TIBCO container registry.

### Optional: NGINX Gateway Fabric for Gateway API

Use this optional path when you want to evaluate Gateway API with NGINX Gateway Fabric. The example installs Gateway API CRDs, installs NGINX Gateway Fabric, and creates a Gateway named `nginx-gateway` in the `nginx-gateway` namespace.

```bash
export TP_GATEWAY_API_ENABLED="true"
...
kubectl get gateway -n ${TP_GATEWAY_NAMESPACE}
kubectl get svc -n ${TP_GATEWAY_NAMESPACE}
```

On EKS, expose the NGINX Gateway Fabric service through the AWS load balancer that its chart creates, or adapt the listener/service model to your organization's AWS load-balancing standard. Create Route 53 records for the CP and DP hostnames after you confirm the gateway service load-balancer hostname. If you want External DNS to manage Gateway API resources, configure External DNS with Gateway API sources in addition to the Ingress source used earlier in this guide.

### Example: Use Gateway API in `tibco-cp-base`

The `tibco-cp-base` chart supports Gateway API by creating `HTTPRoute` resources:
...
In `tibco-cp-base` 1.18, the `hybrid-proxy` chart always generates **two** HTTPRoute rules — `PathPrefix: /infra/tunnel` and `PathPrefix: /` — regardless of DNS mode. Because `router-operator` also has a `PathPrefix: /` rule, both services would conflict if they share the same wildcard hostname. **Simplified DNS + Gateway API therefore requires hostname separation in 1.18:**

| Service | HTTPRoute hostname | Rules generated by chart (1.18) |
|---|---|---|
| `hybrid-proxy` | `${CP_INSTANCE_ID}-tunnel.${TP_BASE_DNS_DOMAIN}` | `PathPrefix: /infra/tunnel` + `PathPrefix: /` |
| `router-operator` | `*.${TP_BASE_DNS_DOMAIN}` | `PathPrefix: /` |

The Gateway controller routes by hostname first: requests to `${CP_INSTANCE_ID}-tunnel.*` always go to `hybrid-proxy`; requests to `admin.*` or `subscription.*` match the wildcard and go to `router-operator`.

> [!NOTE]
> **From `tibco-cp-base` 1.19.0+ (upcoming):** when `dnsDomain == dnsTunnelDomain`, the chart renders only `PathPrefix: /infra/tunnel` for `hybrid-proxy` — no `/` catch-all. Both services can then share `*.${TP_BASE_DNS_DOMAIN}` with path specificity as the sole discriminator.
...
Validate HTTPRoutes after deployment:
...
When registering a Control Tower data plane with Gateway API, select NGINX Gateway Fabric, use GatewayClass `nginx`, and provide Gateway name `${TP_GATEWAY_NAME}` and namespace `${TP_GATEWAY_NAMESPACE}`. Capabilities that support Gateway API such as BW5, BW6, and Flogo endpoint exposure create `HTTPRoute` resources instead of classic `Ingress` resources.
...
**Why:** The `tibco-cp-base` chart (v1.18.0) is the main TIBCO Control Plane Helm chart. It deploys the router, orchestrator, hybrid proxy, and all CP microservices. Generating a named file (rather than passing all values inline) keeps the configuration auditable and re-usable across upgrades.
...
Choose the file that matches your DNS approach and ingress controller. Each file is **self-contained** — one file per combination, one `helm` command.

| DNS approach | Ingress / routing | Values file |
|:-------------|:-----------------|:------------|
| Simplified DNS | Nginx | `aws-tibco-cp-base-simplified-nginx.yaml` |
| Simplified DNS | Traefik | `aws-tibco-cp-base-simplified-traefik.yaml` |
| Simplified DNS | NGINX Gateway Fabric (Gateway API) | `aws-tibco-cp-base-simplified-gateway-api.yaml` |
| Legacy DNS | Nginx | `aws-tibco-cp-base-legacy-nginx.yaml` |
| Legacy DNS | Traefik | `aws-tibco-cp-base-legacy-traefik.yaml` |
| Legacy DNS | NGINX Gateway Fabric (Gateway API) | `aws-tibco-cp-base-legacy-gateway-api.yaml` |

> **Email note for 1.18.0:** Do not add `global.external.emailServerType`, `global.external.emailServer`, `global.external.fromAndReplyToEmailAddress`, `global.external.cronJobReportsEmailAlias`, or `global.external.platformEmailNotificationCcAddresses` to any values file. Configure email in Platform Console after deployment. The `global.tibco.networkPolicy.emailServer` block only controls optional egress NetworkPolicy creation and does not configure any email provider.

Set the variable for your chosen file before running the helm command:

```bash
# Set to your chosen combination — e.g. for simplified DNS + Nginx:
export CP_VALUES_FILE="aws-tibco-cp-base-simplified-nginx.yaml"
```

---

#### 🔷 Simplified DNS + Nginx (Option A)

Tunnel traffic routes on the subscription host under `/infra/tunnel` — no separate tunnel domain or certificate needed.

```bash
cat > aws-tibco-cp-base-simplified-nginx.yaml <(envsubst \
  '${TP_ENABLE_NETWORK_POLICY}, ${TP_CONTAINER_REGISTRY_URL}, ${TP_CONTAINER_REGISTRY_USER},
   ...
   ${TP_VPC_CIDR}, ${TP_SERVICE_CIDR}, ${TP_STORAGE_CLASS_EFS},
  ...
  << 'EOF'
# =============================================================================
# TIBCO CP BASE — Simplified DNS + Nginx Ingress
# One base domain (TP_BASE_DNS_DOMAIN) for all traffic.
# Tunnel: subscription host + /infra/tunnel path (no separate domain needed).
# Certificate: one wildcard *.${TP_BASE_DNS_DOMAIN} ACM cert.
# =============================================================================
hybrid-proxy:
...
global:
  ...
  external:
    ...
    # Tunnel traffic is distinguished by /infra/tunnel path on the Ingress.
...
#### 🔷 Simplified DNS + Traefik (Option B)

Same as Option A with Traefik as the ingress controller. Tunnel traffic routes on `/infra/tunnel`.

```bash
cat > aws-tibco-cp-base-simplified-traefik.yaml <(envsubst \
  '${TP_ENABLE_NETWORK_POLICY}, ${TP_CONTAINER_REGISTRY_URL}, ${TP_CONTAINER_REGISTRY_USER},
   ${TP_CONTAINER_REGISTRY_PASSWORD}, ${CP_INSTANCE_ID}, ${CP_ADMIN_HOST_PREFIX}, ${CP_SUBSCRIPTION},
   ${CP_HYBRID_CONNECTIVITY}, ${TP_BASE_DNS_DOMAIN},
   ${TP_VPC_CIDR}, ${TP_SERVICE_CIDR}, ${TP_STORAGE_CLASS_EFS},
  ...
  << 'EOF'
# =============================================================================
# TIBCO CP BASE — Simplified DNS + Traefik Ingress
# One base domain (TP_BASE_DNS_DOMAIN) for all traffic.
# Tunnel: subscription host + /infra/tunnel path (no separate domain needed).
# Certificate: one wildcard *.${TP_BASE_DNS_DOMAIN} ACM cert.
# =============================================================================
...
> and `PathPrefix: /` for `hybrid-proxy`. To avoid a routing conflict with `router-operator`'s `/` rule,
> assign `hybrid-proxy` a dedicated tunnel subdomain hostname (`${CP_INSTANCE_ID}-tunnel.${TP_BASE_DNS_DOMAIN}`)
> and set `dnsTunnelDomain` to that subdomain. The Gateway controller dispatches by hostname first —
> requests to the tunnel subdomain go to `hybrid-proxy`, requests to the admin/subscription hosts go to
> `router-operator`.
>
> **From `tibco-cp-base` 1.19.0+ (upcoming):** setting `dnsDomain == dnsTunnelDomain` eliminates the
> `/` catch-all, allowing both services to share `*.${TP_BASE_DNS_DOMAIN}` without hostname separation.
>
> The `*.${TP_BASE_DNS_DOMAIN}` wildcard certificate already covers the tunnel subdomain — no extra ACM cert needed.
> Add one Route 53 alias record: `${CP_INSTANCE_ID}-tunnel.${TP_BASE_DNS_DOMAIN}` → Gateway load balancer hostname.
>
...
cat > aws-tibco-cp-base-simplified-gateway-api.yaml <(envsubst \
  '${TP_ENABLE_NETWORK_POLICY}, ${TP_CONTAINER_REGISTRY_URL}, ${TP_CONTAINER_REGISTRY_USER},
   ${TP_CONTAINER_REGISTRY_PASSWORD}, ${CP_INSTANCE_ID}, ${CP_ADMIN_HOST_PREFIX}, ${CP_SUBSCRIPTION},
   ${TP_BASE_DNS_DOMAIN}, ${TP_GATEWAY_CLASS}, ${TP_GATEWAY_NAME}, ${TP_GATEWAY_NAMESPACE},
   ${TP_VPC_CIDR}, ${TP_SERVICE_CIDR}, ${TP_STORAGE_CLASS_EFS},
   ${TP_DB_HOST}, ${TP_DB_NAME}, ${TP_DB_PORT}, ${TP_DB_USERNAME}, ${TP_DB_PASSWORD}, ${TP_DB_SSL_MODE},
   ${TP_EMAIL_SERVER_CIDR}, ${TP_SMTP_PORT},
   ${TP_ADMIN_EMAIL}, ${TP_ADMIN_FIRSTNAME}, ${TP_ADMIN_LASTNAME},
...
#
# The 1.18 chart generates two rules for hybrid-proxy's HTTPRoute:
#   PathPrefix /infra/tunnel → hybrid-proxy:105
#   PathPrefix /             → hybrid-proxy:105
# The router-operator HTTPRoute has:
#   PathPrefix /             → router-operator:100
# Gateway dispatch is by hostname first, so the tunnel subdomain always wins.
#
# Certificate: *.${TP_BASE_DNS_DOMAIN} wildcard covers the tunnel subdomain — no extra cert.
# DNS: add Route 53 alias: ${CP_INSTANCE_ID}-tunnel.${TP_BASE_DNS_DOMAIN} → Gateway LB hostname.
# =============================================================================
hybrid-proxy:
  enabled: true
  ingress:
...
global:
  ...
  external:
    ...
    # The 1.18 chart renders PathPrefix:/infra/tunnel AND PathPrefix:/ for hybrid-proxy.
    # Routing works correctly because the Gateway dispatches by hostname first.
    dnsDomain: "${TP_BASE_DNS_DOMAIN}"
...
Verify the HTTPRoutes generated by the chart:

```bash
kubectl get httproute -n ${CP_INSTANCE_ID}-ns
```

The `hybrid-proxy` HTTPRoute should contain both path rules on the tunnel subdomain:

```yaml
# hybrid-proxy HTTPRoute (chart-generated, tibco-cp-base 1.18)
spec:
  hostnames:
  - "${CP_INSTANCE_ID}-tunnel.${TP_BASE_DNS_DOMAIN}"
  rules:
...
# router-operator HTTPRoute (chart-generated)
spec:
  hostnames:
  - "*.${TP_BASE_DNS_DOMAIN}"
  rules:
...
#### 🔶 Legacy DNS + Nginx (Option D)

Separate domains for router (`TP_MY_DOMAIN`) and tunnel (`TP_TUNNEL_DOMAIN`). Wildcard host covers all subdomains of each domain.

```bash
cat > aws-tibco-cp-base-legacy-nginx.yaml <(envsubst \
  '${TP_ENABLE_NETWORK_POLICY}, ${TP_CONTAINER_REGISTRY_URL}, ${TP_CONTAINER_REGISTRY_USER},
  ...
  << 'EOF'
# =============================================================================
# TIBCO CP BASE — Legacy DNS + Nginx Ingress
# Two separate domains: TP_MY_DOMAIN (router) and TP_TUNNEL_DOMAIN (tunnel).
# Two wildcard ACM certificates required.
# =============================================================================
router-operator:
  tscSessionKey:
    secretName: session-keys
      ...
      - host: '*.${TP_TUNNEL_DOMAIN}'
        paths:
          - path: /
            pathType: Prefix
            port: 105
  # Option B: Dedicated NLB (set ingress.enabled: false and uncomment to use NLB instead)
  # service:
  #   type: LoadBalancer
...
global:
  ...
  external:
    ...
    # Legacy DNS: separate domains for router UI and tunnel traffic.
...
#### 🔶 Legacy DNS + Traefik (Option E)

Same as Option D with Traefik as the ingress controller.

```bash
cat > aws-tibco-cp-base-legacy-traefik.yaml <(envsubst \
  '${TP_ENABLE_NETWORK_POLICY}, ${TP_CONTAINER_REGISTRY_URL}, ${TP_CONTAINER_REGISTRY_USER},
   ${TP_CONTAINER_REGISTRY_PASSWORD}, ${CP_INSTANCE_ID}, ${TP_TUNNEL_DOMAIN}, ${TP_MY_DOMAIN},
   ${TP_VPC_CIDR}, ${TP_SERVICE_CIDR}, ${TP_STORAGE_CLASS_EFS},
   ${TP_DB_HOST}, ${TP_DB_NAME}, ${TP_DB_PORT}, ${TP_DB_USERNAME}, ${TP_DB_PASSWORD}, ${TP_DB_SSL_MODE},
  ...
  << 'EOF'
# =============================================================================
# TIBCO CP BASE — Legacy DNS + Traefik Ingress
# Two separate domains: TP_MY_DOMAIN (router) and TP_TUNNEL_DOMAIN (tunnel).
# Two wildcard ACM certificates required.
# =============================================================================
router-operator:
...
#### 🔶 Legacy DNS + NGINX Gateway Fabric / Gateway API (Option F)

Legacy DNS with separate domains already provides natural hostname separation for Gateway API. `hybrid-proxy` claims `*.${TP_TUNNEL_DOMAIN}` and `router-operator` claims `*.${TP_MY_DOMAIN}` — no path-based workaround needed.

```bash
cat > aws-tibco-cp-base-legacy-gateway-api.yaml <(envsubst \
  '${TP_ENABLE_NETWORK_POLICY}, ${TP_CONTAINER_REGISTRY_URL}, ${TP_CONTAINER_REGISTRY_USER},
   ${TP_CONTAINER_REGISTRY_PASSWORD}, ${CP_INSTANCE_ID}, ${TP_TUNNEL_DOMAIN}, ${TP_MY_DOMAIN},
   ${TP_GATEWAY_CLASS}, ${TP_GATEWAY_NAME}, ${TP_GATEWAY_NAMESPACE},
   ${TP_VPC_CIDR}, ${TP_SERVICE_CIDR}, ${TP_STORAGE_CLASS_EFS},
   ${TP_DB_HOST}, ${TP_DB_NAME}, ${TP_DB_PORT}, ${TP_DB_USERNAME}, ${TP_DB_PASSWORD}, ${TP_DB_SSL_MODE},
   ${TP_EMAIL_SERVER_CIDR}, ${TP_SMTP_PORT},
   ${TP_ADMIN_EMAIL}, ${TP_ADMIN_FIRSTNAME}, ${TP_ADMIN_LASTNAME},
...
# hybrid-proxy   → *.${TP_TUNNEL_DOMAIN}  (HTTPRoute)
# router-operator → *.${TP_MY_DOMAIN}      (HTTPRoute)
#
# Two wildcard ACM certificates required: *.${TP_MY_DOMAIN} and *.${TP_TUNNEL_DOMAIN}.
# =============================================================================
hybrid-proxy:
  enabled: true
  ingress:
...
The `hybrid-proxy` HTTPRoute should contain both path rules on the tunnel domain:
...
Verify the generated file before deploying:
...
Then regenerate the values file (re-run the `cat >` command for your chosen combination from the previous section). The chart's `db_ssl_mode: "require"` instructs the orchestrator to open a TLS-encrypted connection without verifying the server certificate.
...
> **Applies to:** Security-hardened clusters where the pipeline service account cannot create arbitrary namespaces, ClusterRoles, or NetworkPolicy resources (for example, Vodafone TaaS or similar enterprise setups).
>
...
> **CRD pre-apply (skip-crds):** If the cluster does not yet have TIBCO CRDs registered, Helm will print `WARNING: This chart or one of its subcharts contains CRDs. Rendering may fail`. Apply the CRDs manually before running the command above, then add `--skip-crds` to the `helm upgrade` line. See [Troubleshooting — CRD Rendering Warning](#crd-rendering-warning-and---skip-crds-flag).
...
**Error:** Pre-deploy step fails with `nil pointer evaluating interface {}.enabled` referencing `psp.yaml` inside the `tp-cp-prometheus` sub-chart.

**Cause:** PodSecurityPolicies are deprecated and removed in Kubernetes v1.25+ (EKS). The `tp-cp-prometheus` sub-chart evaluates `podSecurityPolicy.enabled`, but when the parent block is completely absent from your values file the Helm template engine crashes with a nil pointer.

**Fix:** Add the following block to your values file to explicitly instantiate and disable PodSecurityPolicy creation:
...
**Cause:** The database password contains special characters (`$`, `%`, `(`, etc.) that conflict with Helm's internal token evaluator. When the value is wrapped in double quotes, Helm misinterprets `$` as the start of a template expression.

**Fix:** Wrap the password in single quotes and escape any dollar sign characters by doubling them (`$$`):

```yaml
# In your values.yaml — use single quotes and $$ for literal dollar signs
global:
  external:
...
**Error:** `yaml: did not find expected key` or structural parsing failures caused by unreplaced `${TOKEN}` strings or misaligned indentation in the `networkPolicy` block.

**Cause:**
1. Pipeline environment tokens such as `${TP_EMAIL_SERVER_CIDR}` were left as literal strings in the values file because the pipeline pre-hook did not inject them.
2. Indentation shifts in the `networkPolicy.endpoints` block collapse YAML map keys into null values, causing downstream rendering failures.

**Fix:**
1. Replace any unreplaced `${TOKEN}` placeholder with an empty string `""` if the variable is not actively injected by a pipeline step.
2. Verify the `networkPolicy` block is correctly indented under `global.tibco`. Disable `createNetworkPolicy` in environments where the pipeline service account lacks RBAC to create NetworkPolicy resources:

```yaml
global:
  tibco:
    createNetworkPolicy: false    # Disable if pipeline SA lacks NetworkPolicy RBAC
    networkPolicy:
      database:
        CIDR: ""
        port: "5432"
      emailServer:
        CIDR: ""                  # Leave empty to skip creating the email egress policy
        port: "587"
      containerRegistry:
        CIDR: ""
        port: "443"
...
**Cause:** By default, the TIBCO Control Plane chart assumes a multi-namespace topology where it can freely create and manage resources across dynamic namespaces for different capabilities and data planes. In security-hardened enterprise clusters (such as Vodafone TaaS), the pipeline service account is typically restricted to a single pre-provisioned namespace.

**Fix:** Add `useSingleNamespace: true` and `createNetworkPolicy: false` under `global.tibco` in your values file. This restricts the entire TIBCO Control Plane footprint — microservices, ingress routes, and jobs — to a single namespace boundary:
...
**Error:** `WARNING: This chart or one of its subcharts contains CRDs. Rendering may fail or contain inaccuracies.` followed by parser failures from sub-charts such as `tp-cp-auditsafe`.

**Cause:** The TIBCO Platform chart bundles Custom Resource Definitions (such as `TibcoRoute`, `Capability`, and Gateway API specifications). When running `helm template` or `helm upgrade` against a cluster where those CRDs are not yet registered, Helm cannot validate sub-chart resource mappings and produces false-positive parse failures.

**Fix:** Apply the CRDs directly to the cluster before running the Helm install, then add `--skip-crds` to the `helm upgrade` command to prevent Helm from re-evaluating the bundled CRD templates:

```bash
# Step 1: Apply the CRDs to the cluster first
# (extract the chart or reference the crds/ directory from the downloaded .tgz)
kubectl apply -f /path/to/tibco-cp-base-1.18.0-extracted/crds/

# Step 2: Install / upgrade with --skip-crds
helm upgrade --install --wait --timeout 1h --create-namespace \
  -n ${CP_INSTANCE_ID}-ns tibco-cp-base tibco-cp-base \
  --labels layer=1 \
  --repo "${TP_TIBCO_HELM_CHART_REPO}" --version "1.18.0" \
...
In an Azure DevOps pipeline, add a dedicated step before the `helm upgrade` step:

```bash
# Step 1: Install/Update the Custom Resource Definitions first
kubectl apply -f /azp/_work/1/helm/tibco-cp-base/crds/

# Step 2: Proceed with validation/install using --skip-crds
helm template tibco-cp-base /azp/_work/1/helm/tibco-cp-base-1.18.0.tgz \
  -f /azp/_work/1/s/your-repo/k8s/live/values.yaml \
  --skip-crds
```
